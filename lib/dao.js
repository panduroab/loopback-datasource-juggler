        ...
        this._adapter().create(modelName, this.constructor._forDB(obj.toObject(true)), function (err, id, rev) {
          ...
          saveDone.call(obj, function () {
            createDone.call(obj, function () {
              ...
              Model.notify('after save', { model: obj }, function(err) {
                callback(err, obj);
...
DataAccessObject.updateOrCreate = DataAccessObject.upsert = function upsert(data, callback) {
  ...
  if (this.getDataSource().connector.updateOrCreate) {
    ...
    Model.notify('before save', { model: inst }, function(err) {
      if (err) return callback(err);

      // FIXME(bajtos) validate the model!
      // TODO(bajtos) DISCUSSION POINT - this is possible a breaking change
    ...
    function done(err, data) {
      ...
      if (err) {
        callback(err, obj);
        if(!err) {
          // TODO(bajtos) Move this to an "after save" hook
...
DataAccessObject.find = function find(query, cb) {
  ...
  var allCb = function (err, data) {
    if (data && data.forEach) {
      ...
      async.eachSeries(
        data,
        function(obj, next) {
          obj.constructor.notify('after load', { model: obj }, next);
        },
        function(err) {
          cb(err, data);
        });
  ...
  var self = this;
  this.notify('before load', { query: query }, function(err) {
...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, cb) {
  ...
  var context = { where: whereIsEmpty(where) ? {} : where };
  Model.notify('before delete', context, function(err) {
    if (err) return cb(err);
    where = context.where;
    ...
    function done(err, data) {
      if (err) return cb(er);
      Model.notify('after delete', context, function(err) {
        cb(err, data);
        if (!err)
...
DataAccessObject.count = function (where, cb) {
  ...
  var Model = this;
  var ctx = { query: { where: where } };
  this.notify('before load', ctx, function(err) {
      if (err) return cb(err);
      ...
      inst.trigger('save', function (saveDone) {
        inst.trigger('update', function (updateDone) {
          ...
          inst._adapter().save(modelName, inst.constructor._forDB(data), function (err) {
            ...
            Model.notify('after save', { model: inst }, function(err) {
...
DataAccessObject.prototype.remove =
  DataAccessObject.prototype.delete =
    DataAccessObject.prototype.destroy = function (cb) {
      ...
      var context = { where: byIdQuery(Model, id).where };
      Model.notify('before delete', context, function(err) {
...
DataAccessObject.prototype.updateAttributes = function updateAttributes(data, cb) {
  ...
  var originalData = inst.toObject(true);
  Model.notify('before save', { model: inst }, function(err) {
    if (err) return cb(err);

    // apply any updates performed by `before save` hooks
    // FIXME(bajtos) low performance for nested objects
    var modifiedData = inst.toObject(true);
    for (var key in modifiedData) {
      if (modifiedData[key] === originalData[key])
        continue;

      var mod = modifiedData[key];
      var orig = originalData[key];
      if (typeof mod === 'object' && typeof orig === 'object' &&
          JSON.stringify(mod) === JSON.stringify(orig))
        continue;

      data[key] = modifiedData[key];

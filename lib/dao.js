...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, options, cb) {
  ...
  if (options.notify === false) {
    doDelete(null, context);
  } else {
    Model.notify('before delete', context, doDelete);
  }

  function doDelete(err, context) {
    if (err) return cb(err);
    where = context.where;
...
DataAccessObject.prototype.remove =
  DataAccessObject.prototype.delete =
    DataAccessObject.prototype.destroy = function (cb) {
      ...
      Model.notify('before delete', context, function(err, ctx) {
        if (err) return cb && cb(err);

        if (!isWhereByGivenId(Model, ctx.where, id)) {
          ...
          Model.deleteAll(ctx.where, { notify: false }, function(err) {
            if (err) return cb && cb(err);
            Model.notify('after delete', context, function(err) {
              return cb && cb(err);
            });

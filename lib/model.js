...
ModelBaseClass.observe = function(operation, listener) {
  if (!this._observers[operation]) {
    ...
    var base = this.base;
    if (base && base._observers) {
      this._observers[operation].push(
        function notifyBaseObservers(context, next) {
          var observers = base._observers[operation];
          base._notifyObservers(this, observers, context, next);
        });
    }
  }
 ...
 *   has finished.
 */
ModelBaseClass.notify = function(operation, context, callback) {
  var handlers = this._observers && this._observers[operation];
  this._notifyObservers(this, handlers, context, callback);
};

ModelBaseClass._notifyObservers = function(self, observers, context, callback) {
  if (!observers || !observers.length) return callback();
  async.eachSeries(
    observers,
    function(fn, next) { fn.call(self, context, next); },
    callback);
}

...
ModelBaseClass.notify = function(operation, context, callback) {
  ...
  this._notifyBase(operation, context, function notifyObservers(err) {
...
ModelBaseClass._notifyBase = function(operation, context, callback) {
  if (this.base && this.base.notify)
    this.base.notify(operation, context, callback);
  else
    callback();
}
